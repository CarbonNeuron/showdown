BEGIN {
    ...
    # Camera basis: forward = normalize(lookAt - camPos)
    ...
    for (j = 0; j < h; j++) {
        # v maps from +half_h (top) to -half_h (bottom)
        v_coord = 1.0 - 2.0 * (j + 0.5) * inv_h
        v_scaled = v_coord * half_h
        for (i = 0; i < w; i++) {
            # u maps from -half_w (left) to +half_w (right)
            u_coord = 2.0 * (i + 0.5) * inv_w - 1.0
            u_scaled = u_coord * half_w

            # Ray direction = normalize(forward + u*right + v*camUp)
            ...
            # Iterative ray tracing with stack
            sp = 0
            ...
            while (sp > 0) {
                ...
                if (hit_ground) {
                    ...
                    # awk int() truncates toward zero, need proper floor
                    if (hx < 0) {
                        _tmp = hx - 1.0
                        fx_c = int(_tmp)
                ...
                for (li = 0; li < NLIGHT; li++) {
                    ...
                    # Shadow ray origin (offset by normal * epsilon)
                    sox = hx + nx_n * EPS
                    soy = hy + ny_n * EPS
                    soz = hz + nz_n * EPS
                    ...
                    if (ndl > 0) {
                        ...
                        # Specular (Phong) - white highlight
                        if (h_spec > 0) {
                            dn2 = 2.0 * ndl
                            rlx = -lx + dn2*nx_n
                            rly = -ly + dn2*ny_n
                            ...
                            if (rdv > 0) {
                                sp_val = exp(log(rdv) * h_spec) * intensity
            ...
            # Gamma correction: pow(x, 1/2.2) = exp(log(x) * (1/2.2))
            if (fin_r > 0) rb = int(exp(log(fin_r) * GAMMA_INV) * 255 + 0.5); else rb = 0
            if (fin_g > 0) gb = int(exp(log(fin_g) * GAMMA_INV) * 255 + 0.5); else gb = 0
            if (fin_b > 0) bb = int(exp(log(fin_b) * GAMMA_INV) * 255 + 0.5); else bb = 0
